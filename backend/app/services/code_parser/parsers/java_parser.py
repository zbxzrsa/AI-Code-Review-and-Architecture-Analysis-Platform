"""
Java代码解析器
实现Java代码的解析、AST生成、CFG/DFG提取和度量计算
"""
import asyncio
...
class JavaParser(BaseParser):
    ...
    async def generate_ast(self, code: str) -> Dict[str, Any]:
        ...
        try:
            # 使用Tree-sitter解析代码
            raw_ast = await ts_parser.parse_code(code, "java")
            
            # 标准化AST
            standardized_ast = standardize_ast(raw_ast, "java")
    ...
    async def _extract_control_flow(self, ast: Dict[str, Any], cfg: Dict[str, Any]) -> None:
        ...
        if "children" in ast:
            for child in ast["children"]:
                ...
                # 处理控制流结构
                if node_type in ["if_statement", "for_statement", "while_statement", "try_statement", "switch_statement"]:
                    # 创建条件/循环节点
                ...
                # 处理方法定义
                    ...
                    if "children" in child:
                        for method_child in child["children"]:
                            if method_child.get("type") == "block":
                                await self._extract_control_flow(method_child, cfg)
    ...
    def _get_method_name(self, node: Dict[str, Any]) -> str:
        """
        从方法定义节点获取方法名
    ...
    async def _extract_variables(self, node: Dict[str, Any], variables: Dict[str, Dict[str, Any]]) -> None:
        ...
        # 处理变量定义
        if node_type in ["variable_declarator", "field_declaration"]:
            if "children" in node:
                for child in node["children"]:
                    if child.get("type") == "identifier":
    ...
    async def calculate_metrics(self, ast: Dict[str, Any], cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算方法数量
            metrics["method_count"] = await self._count_methods(ast)
    ...
    async def _calculate_complexity_recursive(self, node: Dict[str, Any], complexity: int) -> int:
        ...
        # 增加复杂度的节点类型
        if node_type in ["if_statement", "else_clause", "for_statement", "while_statement", "catch_clause", "case_statement"]:
            complexity += 1
    ...
    async def _count_methods(self, ast: Dict[str, Any]) -> int:
        """
        计算方法数量
        ...
        # 检查当前节点
        if ast.get("type") == "method_declaration":
            count += 1
        
        # 递归处理子节点
        if "children" in ast:
            for child in ast["children"]:
                count += await self._count_methods(child)
    ...
    async def _count_classes(self, ast: Dict[str, Any]) -> int:
        ...
        # 检查当前节点
        if ast.get("type") == "class_declaration":
            count += 1
