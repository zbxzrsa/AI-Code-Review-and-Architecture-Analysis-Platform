"""
JavaScript代码解析器
实现JavaScript代码的解析、AST生成、CFG/DFG提取和度量计算
"""
import asyncio
...
class JavaScriptParser(BaseParser):
    ...
    async def generate_ast(self, code: str) -> Dict[str, Any]:
        ...
        try:
            # 使用Tree-sitter解析代码
            raw_ast = await ts_parser.parse_code(code, "javascript")
            
            # 标准化AST
            standardized_ast = standardize_ast(raw_ast, "javascript")
                ...
                # 处理函数定义
                    ...
                    if "children" in child:
                        for func_child in child["children"]:
                            if func_child.get("type") == "statement_block":
    ...
    async def _calculate_complexity_recursive(self, node: Dict[str, Any], complexity: int) -> int:
        ...
        # 增加复杂度的节点类型
        if node_type in ["if_statement", "else_clause", "for_statement", "while_statement", "catch_clause", "case_statement", "ternary_expression"]:
            complexity += 1
    ...
    async def _count_functions(self, ast: Dict[str, Any]) -> int:
        ...
        # 检查当前节点
        if ast.get("type") in ["function_declaration", "arrow_function", "method_definition"]:
            count += 1
