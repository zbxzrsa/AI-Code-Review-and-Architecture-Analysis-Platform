"""
Go代码解析器
实现Go代码的解析、AST生成、CFG/DFG提取和度量计算
"""
import asyncio
...
class GoParser(BaseParser):
    ...
    async def generate_ast(self, code: str) -> Dict[str, Any]:
        ...
        try:
            # 使用Tree-sitter解析代码
            raw_ast = await ts_parser.parse_code(code, "go")
            
            # 标准化AST
            standardized_ast = standardize_ast(raw_ast, "go")
    ...
    async def _extract_control_flow(self, ast: Dict[str, Any], cfg: Dict[str, Any]) -> None:
        ...
        if "children" in ast:
            for child in ast["children"]:
                ...
                # 处理控制流结构
                if node_type in ["if_statement", "for_statement", "switch_statement", "select_statement"]:
                    # 创建条件/循环节点
    ...
    async def _extract_variables(self, node: Dict[str, Any], variables: Dict[str, Dict[str, Any]]) -> None:
        ...
        # 处理变量定义
        if node_type in ["short_var_declaration", "var_spec"]:
            if "children" in node:
                for child in node["children"]:
                    if child.get("type") == "identifier":
    ...
    async def _calculate_complexity_recursive(self, node: Dict[str, Any], complexity: int) -> int:
        ...
        # 增加复杂度的节点类型
        if node_type in ["if_statement", "for_statement", "switch_statement", "select_statement", "communication_case"]:
            complexity += 1
    ...
    async def _count_functions(self, ast: Dict[str, Any]) -> int:
        ...
        # 检查当前节点
        if ast.get("type") == "function_declaration":
            count += 1
