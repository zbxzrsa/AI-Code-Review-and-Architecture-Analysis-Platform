...
class PredictiveCacheManager:
    ...
    async def _load_historical_patterns(self):
        ...
        for key, logs in pattern_groups.items():
            ...
            await self._build_access_patterns(logs, tenant_id, repo_id)
        ...
        for key in pattern_keys:
            ...
            if pattern_data:
                ...
                pattern = AccessPattern(
                    ...
                    last_access=datetime.fromisoformat(data.get("last_access", datetime.utcnow().isoformat())),
                ...
                # 计算访问频率
                first_access_str = data.get("first_access")
                if first_access_str:
                    try:
                        first_access = datetime.fromisoformat(first_access_str)
                        days_since_first = (datetime.utcnow() - first_access).days + 1
                        pattern.access_frequency = pattern.access_count / days_since_first
                    except ValueError:
                        pattern.access_frequency = 0.0
                else:
                    pattern.access_frequency = 0.0
    ...
    async def _build_access_patterns(
        self,
        logs: List[Dict],
        tenant_id: str,
        repo_id: str
        ...
        for log in logs:
            file_accesses[log["file_path"]].append(log)
